...
const CommunitySectionCmtDetails: React.FC = () => {
    ...
    const [getImagesArray, setImagesArray] = useState<ImageType[]>([]);
    ...
    console.log('loginIdBased',loginIdBased);
    useEffect(() => {
    ...
    const MAX_FILE_SIZE = 1000 * 1024; // 500 KB
    const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        await handleUploadImages(validFiles);
    ...
    const handleUploadImages = async (files: File[]) => {
        ...
        try {
            ...
            if (response.data.status === "success") {
                ...
                const newImages = response.data?.data || []; // adjust based on API
    ...
    const handleBlocked = (blockId:any ,BlockPostId: any, BlockedUserId: any,checked: boolean) => {
        ...
        console.log(` User ID: ${BlockedUserId}  blockedBy ID: ${userId}  get :${getBlockedUserId}`);
    };
        const handleSubmitReport = async (getBlockedUserId:any,getBlockedId:any) => {
             console.log('getBlockedId',getBlockedId);
             if (!getBlockedId || deleting) return;
            ...
            setComments((prev) =>
                prev.filter((c) => c.id !== getBlockedId)
            );
            setFollowingBy((prev) => {
                if (!prev) return prev;
    ...
    const isOwner = userId && getfollowingBy?.post_by_userid && userId === getfollowingBy.post_by_userid;
    const fetchPostDetail = async (slug: any) => {
        ...
        try {
            ...
            console.log('tets getImagesArray',getImagesArray);
                            ...
                            <div className="single-feed position-relative">
                                ...
                                {messageBlockComment && <div style={{color:'#FC673C' , textAlign:'left',margin:'0px'}}>{messageBlockComment}</div>}
                                {
                                    ...
                                    getComments.slice(0, visibleCount).map((cmt: any, index: number) => (
                                        <div key={cmt.id ||index}  className="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                            <div  className="testimonial-single position-relative">
                                                <div className="testimonial-head d-flex w-100 align-items-center position-relative">
                                                    ...
                                                    <div className="test-head">
                                                        ...
                                                        <p className="mb-0">{cmt.id} {cmt.commentText ?? 'N/A'}</p>
                                                    </div>
