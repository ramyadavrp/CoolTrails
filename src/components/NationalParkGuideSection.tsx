...
const NationalParkGuideSection: React.FC = () => {
    ...
    useEffect(()=>{
        ...
        const filteredCountries = useMemo(() => {
            ...
            return getParkList.filter((p) => p.country.toLowerCase() === country.toLowerCase());
            }, [getParkList, country]);
    ...
    if (errorsNParks) return <p>{errorsNParks}</p>;
    if (getParkList.length === 0) {
        return (
            <div className="d-flex justify-content-center align-items-center" style={{ height: "60vh" }}>
            <p className="text-center">NO National Parks found.</p>
            </div>
        );
    }
    return (
                        ...
                        
                            {
                                getPark.length > 0 ?(
                                    getPark.map((pk:any, index:number)=>{
                                        ...
                                        return(
                                            <div key={index} className="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12">
